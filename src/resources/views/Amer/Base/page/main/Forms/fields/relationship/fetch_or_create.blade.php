...
<?php
    ...
    if(isset($field['data_source'])){
        ...
        $field['data_source'] =url($Amer->route.'/fetch/'.$routeEntity);
    }
...
@include(fieldview('inc.wrapper_start'))
        ...
        @include(fieldview('relationship.inline_create_modal'), ['field' => $field])
        @endif
<select
        name="{{ $fieldName }}"
        id="{{ $field['name']  }}"
        ...
        if (!element.hasClass("select2-hidden-accessible"))
            {
                ...
                bpFieldInitSelect2FromAjax(uniqueid);
            }
    }
    </script>
    @endLoadOnce
