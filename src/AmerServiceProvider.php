...
class AmerServiceProvider extends ServiceProvider
{
    ...
    public function boot(Router $router): void
    {
        $this->loadConfigs();
        $this->loadviewfiles();
        $this->loadTranslationsFrom(Config('amer.package_path')."lang", 'AMER');
        $this->setDisks();
    ...
    function loadviewfiles() {
        ...
        $this->loadViewsFrom(Config('amer.package_path').'resources/views/Amer', 'Amer');
    ...
    function setDisks(){
        ...
        //set public disk
        app()->config['filesystems.disks.'.config('amer.root_disk_name')] = [
            'driver'=>'local',
    ...
    {
        $routepath=getallfiles(config('amer.package_path').'route/');
        foreach($routepath as $path){
            $this->loadRoutesFrom($path);
        }
        $routepath=getallfiles(base_path('Routes/Amer'));
        foreach($routepath as $path){
            $this->loadRoutesFrom($path);
        }
    }
