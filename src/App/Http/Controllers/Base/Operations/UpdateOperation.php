...
trait UpdateOperation
{
    ...
    protected function setupUpdateDefaults()
    {
        ...
        $this->Amer->operation(['list', 'show'], function () {
            ...
            if($req->bearerToken() == ""){
                ...
                $this->Amer->addButton('line', 'update', 'view', 'buttons.update', 'end');
            }
    ...
    public function edit($id)
    {
        ...
        $this->Amer->hasUploadFields('update') ? $this->Amer->set('isMultipart',true) : $this->Amer->set('isMultipart',false);
        $this->Amer->set('currentOperation','update');
        $layout='FormLayout';
        $return=[
            'layout'=>$layout,
            'settings'=>$this->Amer->settings(),
            'title'=>$this->Amer->getTitle() ??  trans('AMER::Amer.edit').' '.$this->Amer->entity_name,
            'routeList'=>$this->Amer->routelist,
            'currentOperation'=>$this->Amer->getcurrentOperation(),
            'entry'=>$this->Amer->getEntryWithLocale($id)
        ];
        return \AmerHelper::returnLayoutData($return);
    ...
    public function update()
    {
        $this->Amer->hasAccessOrFail('update');
        $req=$this->Amer->getRequest();
        if (str_contains($req->header('Content-Type'), 'multipart/form-data')) {
            dd($request->path());
            dd($req->getContent());
        }
