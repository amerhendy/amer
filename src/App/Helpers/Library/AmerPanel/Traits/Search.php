...
trait Search
{
    ...
    */
    ...
    /**
     * Add conditions to the query for a particular search term.
     *
    ...
    /**
     * Apply the search logic for each column.
     */
    public function applySearchLogicForColumn($query, $column, $searchTerm)
    {
        ...
        if ($column['tableColumn']) {
            $searchOperator = config('Amer.operations.list.searchOperator', 'like');
    ...
    /**
     * Apply the datatables order to the query.
     */
    ...
    public function getResponsiveTable()
    {
        ...
        return config('Amer.Baselist.responsiveTable');
    }
    ...
    public function getPersistentTable()
    {
        ...
        return config('Amer.Baselist.persistentTable');
    }
    ...
    public function getPersistentTableDuration()
    {
        ...
        return config('Amer.Baselist.persistentTableDuration', false);
    }
    ...
    public function getRowViews($entry, $rowNumber = false)
    {
        ...
        if ($this->getOperationSetting('detailsRow')) {
            $details_row_button = \View::make(listview('columns.inc.details_row_button'))
                                           ->with('Amer', $this)
                                           ->with('entry', $entry)
                                           ->with('row_number', $rowNumber)
                                           ->render();
    ...
    private function getCellViewName($column)
    {
		...
		if($column['type'] !== 'text'){
            //dd($column['type'],ViewNamespaces::getFor('columns'));
        }
    ...
    private function renderCellView($view, $column, $entry, $rowNumber = false)
    {
        ...
        if (! view()->exists($view)) {
            $view = listview('columns.text'); // fallback to text column
        }
