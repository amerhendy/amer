...
trait SaveActions
{
    ...
    {
        return config('Amer.Base'.$this->getCurrentOperation().'.defaultSaveAction', 'save_and_back');
    }
    ...
    public function getFallBackSaveAction()
    {
        ...
        //if there is an higher action and that action is not the Amer default higher one `save_and_back` we return it.
    ...
    /**
     * Allow developers to register save action into Amer.
     *
    ...
    public function addSaveAction(array $saveAction)
    {
        ...
        $saveAction['redirect'] = $saveAction['redirect'] ?? function ($Amer, $request, $itemId) {
            return $request->has('_http_referrer') ? $request->get('_http_referrer') : $Amer->route;
        };
    ...
    /**
     * Alias function of replaceSaveActions() for Amer consistency.
     *
     * @param  array  $saveActions
     * @return void
     */
    ...
    /**
     * Return the ordered save actions to use in the Amer panel.
     *
     * @return array
     */
    ...
    /**
     * Gets the current save action for this Amer.
     *
    ...
    public function setSaveAction($forceSaveAction = null)
    {
        ...
        $showBubble = $this->getOperationSetting('showSaveActionChange') ?? config('Amer.Base'.$this->getCurrentOperation().'.showSaveActionChange') ?? true;

        if (
            $showBubble &&
            session($this->getCurrentOperation().'.saveAction', 'save_and_back') !== $saveAction
        ) {
            \Alert::info(trans('AMER::actions.save_action_changed_notification'))->flash();
        }
    ...
    /**
     * This functions register default save actions into Amer.
     *
     * @return array
     */
        ...
        $defaultSaveActions = [
            [
                ...
                'visible' => function ($amer) {
                    return $amer->hasAccess('list');
                },
                'redirect' => function ($amer, $request, $itemId = null) {
                    return $request->request->has('_http_referrer') ? $request->request->get('_http_referrer') : $amer->route;
                },
                'button_text' => trans('AMER::actions.save_action_save_and_back'),
            ],
            [
                'name' => 'save_and_edit',
                'visible' => function ($amer) {
                    return $amer->hasAccess('update');
                },
                'redirect' => function ($amer, $request, $itemId = null) {
                    $itemId = $itemId ?: $request->request->get('id');
                    $redirectUrl = $amer->route.'/'.$itemId.'/edit';
                ...
                'referrer_url' => function ($amer, $request, $itemId) {
                    return url($amer->route.'/'.$itemId.'/edit');
                },
                'button_text' => trans('AMER::actions.save_action_save_and_edit'),
            ],
            [
                'name' => 'save_and_new',
                'visible' => function ($amer) {
                    return $amer->hasAccess('create');
                },
                'redirect' => function ($amer, $request, $itemId = null) {
                    return $this->route.'/create';
                },
                'button_text' => trans('AMER::actions.save_action_save_and_new'),
            ],
        ];
