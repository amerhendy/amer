...
trait Query
{
     ...
     *
     * Examples:
     * $this->Amer->addClause('active');
     * $this->Amer->addClause('type', 'car');
     * $this->Amer->addClause('where', 'name', '=', 'car');
     * $this->Amer->addClause('whereName', 'car');
     * $this->Amer->addClause('whereHas', 'posts', function($query) {
     *     $query->activePosts();
     * });
     *
    ...
    private function getCountFromQuery(Builder $query)
    {
        ...
        $AmerQuery = $query->toBase()->clone();

        $modelTable = $this->model->getTable();

        // create an "outer" query, the one that is responsible to do the count of the "Amer query".
        $outerQuery = $AmerQuery->newQuery();
        ...
        foreach ($AmerQuery->columns ?? [] as $column) {
        ...
        // this subquery is the "main Amer3 query" without some properties:
        ...
        $subQuery = $AmerQuery->cloneWithout(['columns', 'orders', 'limit', 'offset']);

        // select minimum possible columns for the count
        $minimumColumns = ($AmerQuery->groups || $AmerQuery->havings) ? '*' : $modelTable.'.'.$this->model->getKeyName();
        ...
        //dump($AmerQuery->getColumns(), get_class($AmerQuery), get_class($subQuery));
        foreach ($AmerQuery->getRawBindings() as $type => $binding) {
