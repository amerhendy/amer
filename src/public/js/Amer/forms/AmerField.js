    ...
    class AmerField {
        constructor(name) {
            ...
            if (this.wrapper.length === 0) {
                console.error(`AmerField error! Could not select WRAPPER for "${this.name}"`);
            }

            // Validate that the field has been found
            if(this.$input.length === 0) {
                console.error(`AmerField error! Could not select INPUT for "${this.name}"`);
            }
        ...
        onChange(closure) {
            ...
            if(this.isSubfield) {
                window.Amer.subfieldsCallbacks[this.parent.name] ??= [];
                window.Amer.subfieldsCallbacks[this.parent.name].push({ closure, field: this });
                this.wrapper.trigger('AmerField:subfieldCallbacksUpdated');
                return this;
            }
        ...
        change() {
            if(this.isSubfield) {
                window.Amer.subfieldsCallbacks[this.parent.name]?.forEach(callback => callback.triggerChange = true);
            } else {
        ...
        show(value = true) {
            ...
            let event = new Event(`AmerField:${value ? 'show' : 'hide'}`);
            this.input?.dispatchEvent(event);
            return this;
        }
        ...
        enable(value = true) {
            ...
            let event = new Event(`AmerField:${value ? 'enable' : 'disable'}`);
            this.input?.dispatchEvent(event);
            return this;
        }
        disable(value = true) {
        ...
        require(value = true) {
            ...
            let event = new Event(`AmerField:${value ? 'require' : 'unrequire'}`);
            this.input?.dispatchEvent(event);
            return this;
        }
        ...
        subfield(name, rowNumber = false) {
            let subfield = new AmerField(this.name);
            subfield.name = name;
    ...
    window.Amer = {
        ...
        field: name => new AmerField(name),

        // Create all fields from a given name list
        fields: names => names.map(window.Amer.field),
    };
    (function( $ ){
    $('a[data-bs-toggle="tab"]').on('click', function (e) {
        ...
        if (activeTabs) {
            ...
            $.each(activeTabs, function (index, element) {
                var triggerEl = document.querySelector('[data-bs-toggle="tab"][href="' + element + '"]')
