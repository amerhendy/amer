...
trait Input
{
    ...
    {
        $AmerFields = $relationDetails['AmerFields'] ?? [];
     ...
     * @return array
     */
    public function getDirectInputsFromInput($input, $model = false, $fields = [], $relationMethod = false)
    {
     ...
     *
     * So when editing current model Amer user have two fields
     * - address.line_1
     * - address.country
     ...
     * @param  array  $AmerFields  - present when getting the relation details for other relations.
     * @param  mixed  $relationMethod
     * @return array The formatted relation details.
     */
    public function getRelationDetailsFromInput($input, $AmerFields = [], $relationMethod = false)
    {
        // main entity
        if (empty($AmerFields)) {
            ...
            $relationFields = $AmerFields;

            foreach ($AmerFields as $key => $AmerField) {
                if (isset($AmerField['subfields'])) {
                    foreach ($AmerField['subfields'] as $AmerSubField) {
                        if (isset($AmerSubField['relation_type'])) {
                            $relationFields[] = $AmerSubField;
                        }
                    }
                }
            }
