...
use Amerhendy\Employment\App\Helpers\Library\AmerPanel\AmerFilter;
...
trait Filters
{
    ...
    /**
     * Add a filter to the Amer table view.
     *
    ...
    /**
     * Add a filter to the AmerPanel object using the Settings API.
    ...
    protected function addFilterToCollection($options, $values = false, $filterLogic = false, $fallbackLogic = false)
    {
        ...
        $filter = new AmerFilter($options, $values, $filterLogic, $fallbackLogic);
    ...
    /**
     * Add a filter by specifying the entire AmerFilter object.
     * The filter logic does NOT get applied.
     *
     * @param  AmerFilter  $object
     */
    public function addAmerFilter($object)
    {
     ...
     * Apply the filter.
     *
     * @param  AmerFilter  $filter
     * @param  ParameterBag|array|null  $input
     */
    public function applyFilter(AmerFilter $filter, $input = null)
    {
        $filter->apply($input);
    }
    ...
    {
        $AmerFilter = $this->getFilter($name);

        return $AmerFilter instanceof AmerFilter && $AmerFilter->isActive();
    }
     ...
     * @return AmerFilter The filter that has suffered modifications, for daisychaining methods.
     */
    public function modifyFilter($name, $modifications)
    {
        ...
        if (! $filter) {
            abort(500, 'Amer Filter "'.$name.'" not found. Please check the filter exists before you modify it.');
        }
    ...
    /**
     * Create and return a AmerFilter object for that attribute.
     *
     * Enables developers to use a fluent syntax to declare their filters,
     * in addition to the existing options:
     * - Amer::addFilter(['name' => 'price', 'type' => 'range'], false, function($value) {});
     * - Amer::filter('price')->type('range')->whenActive(function($value) {});
     *
    ...
    {
        return new AmerFilter(compact('name'), null, null, null);
    }
}
