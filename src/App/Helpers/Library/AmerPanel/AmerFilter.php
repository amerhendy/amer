...
use Amerhendy\Employment\ViewNamespaces;
use Closure;
...
class AmerFilter
{
    ...
    public $viewNamespace = 'Amer::filters';
    ...
    {
        // if filter exists
        if ($this->Amer()->hasFilterWhere('name', $options['name'])) {
            $properties = get_object_vars($this->Amer()->firstFilterWhere('name', $options['name']));
            ...
            $this->label = $options['label'] ?? $this->Amer()->makeLabel($this->name);
        ...
        if ($this->Amer()->getRequest()->has($this->name)) {
            $this->currentValue = $this->Amer()->getRequest()->input($this->name);
        }
    }
     ...
     * the Amer query has its results filtered, according to the Request.
     *
    ...
    public function apply($input = null)
    {
        ...
        $input = $input ?? new ParameterBag($this->Amer()->getRequest()->all());
     ...
     * as configured in the config/Amer.php file.
     *
     * @return array
     */
    public function getNamespacedViewWithFallbacks()
    {
        ...
        if ($this->viewNamespace != 'Amer::filters') {
    ...
    /**
     * Create a AmerFilter object with the parameter as its name.
     *
    ...
    {
        $this->Amer()->removeFilter($this->name);
    }
    ...
    public function forget($attribute)
    {
        ...
        $this->Amer()->replaceFilter($this->name, $this);
    ...
    {
        $this->Amer()->moveFilter($this->name, 'after', $destination);
    ...
    {
        $this->Amer()->moveFilter($this->name, 'before', $destination);
    ...
    {
        $this->Amer()->moveFilter($this->name, 'before', $this->Amer()->filters()->first()->name);
    ...
    /**
     * The path to the blade views directory where the filter file will be found. Ex: 'Amer::filters'
    ...
    private function Amer()
    {
        return app()->make('Amer');
    }

    /**
     * Set the value for a certain attribute on the AmerFilter object.
     *
    ...
    /**
     * Replace all field options on the AmerFilter object
    ...
    /**
     * Update the global AmerPanel object with the current field options.
     *
     * @return AmerFilter
     */
    private function save()
    {
        ...
        if ($this->Amer()->hasFilterWhere('name', $key)) {
            $this->Amer()->modifyFilter($key, (array) $this);
        } else {
            $this->Amer()->addAmerFilter($this);
        }
    ...
    private function applyDefaultLogic($name, $operator, $input = null)
    {
        ...
        switch ($operator) {
            ...
            case '<':
            case '<=':
                $this->Amer()->addClause('where', $name, $operator, $input[$name]);
                break;
     ...
     * - that filter be updated inside the global AmerPanel object;.
     *
