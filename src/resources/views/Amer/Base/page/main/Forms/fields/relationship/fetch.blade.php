...
if($field['name'] == 'Experience_id'){
    ...
    $field['data_source'] = Route('api.alldata',$field['data_source']);
                        ...
                        if(is_array($field['attribute'])){
                            ...
                            foreach($newCurrentca as $a=>$item){
                                foreach(array_keys($item) as $key){
                                    if(in_array($key,$field['attribute'])){
                                        if($field['name'] == 'Experience_id'){
                                            if($item['time'] == 0){
                                                $item['time']="لا يتطلب خبرة سابقة";
                                                unset($item['type']);
                                            }else{
                                                if($item['type'] == 0){
                                                    $item['type']="قضاء مدة بينية قدرها (";
                                                    $item['time']=$item['time'].") سنوات  فى الدرجة الادنى";
                                                }
                                                if($item['type'] == 1){
                                                    $item['type']="خبره في مجال العمل (";
                                                    $item['time']=$item['time'].") سنوات";
                                                }
                                            }
                                        }
                                        if(!isset($field['attribute-break'])){$field['attribute-break'] = " - ";}
                                            $resulta=[];
...
@include('layout.fields.inc.wrapper_start')
    ...
    <select
        ...
        data-field-attribute="{{ isset($field['attribute'])?json_encode(Arr::wrap($field['attribute'])): json_encode([]) }}"
        ...
        var $select2Settings = {
                ...
                ajax: {
                    ...
                    data: function (params) {
                        var sentdata={q: params.term,
                                page: params.page,};
                        if ($includeAllFormFields) {sentdata['form']=form.serializeArray();}
                        if($dependencies.length !== 0){
                            sentdata['dependencies']={};
                        }
