...
use Amerhendy\Amer\App\Helpers\Library\AmerPanel\AmerField;
...
trait Fields
{
    ...
    /**
     * Get the Amer fields for the current operation with name processed to be usable in HTML.
     *
     * @return array
     */
     ...
     * Everything else Amer.Base can probably guess. This method makes sure  the
    ...
    /**
     * When field is a relationship, Amer will try to guess some basic attributes from the relation.
     *
     * @param  array  $field
     * @return array
     */
    public function makeSureFieldHasRelationshipAttributes($field)
    {
        ...
        $field = $this->makeSureFieldHasType($field);
        $field = $this->setFieldDataSource($field);
        return $field;
    }
    function setFieldDataSource($field){
        if(!isset($field['data_source'])){return $field;}
        if (!\Route::has($field['data_source'])) {return $field;}
     ...
     * (once by Amer in front-end, once by Laravel Attribute Casting).
     *
     * @param  array  $input
     * @param  mixed  $model
     * @return array
    ...
    /**
     * Order the Amer fields. If certain fields are missing from the given order array, they will be
    ...
    public function hasUploadFields()
    {
        $types=['file','upload'];
        $fields = $this->getCleanStateFields();
        $upload_fields = Arr::where($fields, function ($value, $key) use($types) {
            ...
            }elseif ($value['type'] == 'file' || $value['type'] == 'upload') {
                return true;
            }elseif(Arr::where($types, function ($val, $k) use($value){
                return \Str::contains($value['name'], $val);
            })){
                return true;
            }
    ...
    {
        $ffl=Arr::flatten(Arr::pluck($this->getCleanStateFields(), 'name'));
        foreach($ffl as $a=>$b){
            if(\Str::contains($b,',')){
                $ods=$a;
            }
        ...
        if(isset($ods)){
            $key=$ods;
            $val=$ffl[$ods];unset($ffl[$ods]);
            $val=explode(',',$val);
            foreach ($val as $k => $v) {
                array_splice( $ffl, $key, 0, $v );
            }
        }
        return $ffl;
    ...
    public function getStrippedSaveRequest($request)
    {
        ...
        // eg. \App\Http\Requests\Amer StrippedRequest
    ...
    /**
     * Create and return a AmerField object for that field name.
     *
     ...
     *
     * And if the developer uses the AmerField object as Field in their AmerController:
